...
public class WorkflowGeneratorCommandLine
{
	...
	public void processSingleWorksheet(ComputeBundle bundle, Hashtable<String, String> userValues,
			String applicationName /* should be unique somehow? */, String backend, String templateDir)
	...
	private List<Tuple> targetWorksheet(List<Tuple> worksheet, String target)
	{
		...
		for (Tuple t : worksheet)
		{
			...
			String key = t.getString(target);

			if (wsset != null && wsset.containsKey(key))
			{
			...
			// put tuple set back in worksheet set
			wsset.put(key, tupleset);
		}

		// put tuples in worksheet
	...
	private void processWorkflowElement(WorkflowElement workflowElement) throws ParseException, IOException
	{

		// get protocol and find its targets
		ComputeProtocol protocol = findProtocol(workflowElement.getProtocol_Name());
		Vector<String> targets = weaver.parseHeaderElement(ParameterWeaverCommandLine.READ_TARGETS,
				protocol.getScriptTemplate());

		// get current worksheet
		List<Tuple> worksheet = computeBundle.getUserParameters();

		// derive worksheet with appropriate targets, given this type
		worksheet = targetWorksheet(worksheet, targets.get(0));

		targetID = 0;
		for (Tuple target : worksheet)
		{
			...
			{
				if (computeFeature.getDefaultValue() == null) continue;
				...
				{
					weavingValues.put(computeFeature.getName(),
							computeFeature.getDefaultValue() != null ? computeFeature.getDefaultValue() : "");
				}
			}

			// process workflow element parameters
			List<WorkflowElementParameter> workflowElementParameters = findWorkflowElementParameters(workflowElement
					.getName());
	...
	private List<WorkflowElementParameter> findWorkflowElementParameters(String name)
	{
		...
		{
			if (p.getWorkflowElement_Name().equalsIgnoreCase(name)) result.add(p);
		}
		return result;
	}
	...
	private ComputeParameter findComputeFeature(String targetName)
	{
		...
		{
			if (f.getName().equalsIgnoreCase(targetName)) return f;
		}
		return null;
	}

	private void generateComputeApplication(WorkflowElement workflowElement, ComputeProtocol protocol,
			Hashtable<String, String> weavingValues, Vector<ComputeParameter> featuresToDerive) throws IOException,
			ParseException
	{
		...
		for (String previousWorkflowElement : strPreviousWorkflowElements)
		{
			...
			if (strDependancy.equalsIgnoreCase("")) strDependancy += "$job_" + jobSubmitID;
			else
				strDependancy += ":$job_" + jobSubmitID;
		}

		if (!strDependancy.equalsIgnoreCase("")) strDependancy = "-W depend=afterok:" + strDependancy;
		...
		// scripts depends on previous scripts
		{
	...
	private Script makeJDLScript(String scriptID, String scriptRemoteLocation, String template,
			Hashtable<String, String> weavingValues)
	{
		...
		String executablesTop = weaver.processGridHeader(ParameterWeaverCommandLine.DO_EXECUTABLE, template,
				weavingValues);
