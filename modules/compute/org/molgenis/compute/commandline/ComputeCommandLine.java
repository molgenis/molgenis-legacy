...
public class ComputeCommandLine {
	...
	private void generateJobs() throws Exception {
		...
		for (WorkflowElement wfe : computeBundle.getWorkflowElements()) {
			...
			for (Tuple work : folded) {
				...
				// FIXME: Here I make queue dependent on walltime and memory per node..., which is specifically for Millipede..
				...
				for (String previousStep : wfe.getPreviousSteps_Name()) {
					...
					for (String target : wfeProtocol.getIterateOver_Name()) {
						if (work.getObject(target) instanceof List) {
							...
							// BUG? What if user puts lists of different length in worksheet?
							break;
						}
					}
		...
		/*
		 * ComputeJob job = new ComputeJob(); job.setName(getworkflowfilename()); job.setInterpreter("bash");
		 * 
		 * // if walltime, cores, mem not specified in protocol, then use value from worksheet job.setWalltime("00:00:10"); job.setCores(1); job.setMem(1);
		 * 
		 * // final job is dependent on all other jobs Set<String> dependencies = new HashSet<String>(); for (ComputeJob cj : this.jobs) { dependencies.add(cj.getName()); }
		 * job.getPrevSteps_Name().addAll(dependencies);
		 * 
		 * // add the script job.setComputeScript("touch $PBS_O_WORKDIR" + File.separator + getworkflowfilename() + ".finished");
		 * 
		 * this.jobs.add(job);
		 * 
		 * // print("compute parameters: " + computeBundle.getComputeParameters().toString()); // print("user parameters: " + computeBundle.getUserParameters()); // print("full worksheet: " +
		 * computeBundle.getWorksheet());
		 */
	}
	 ...
	 **/
	/*
	 * private void generateRite() throws RiteException { MongoRecipeStore msr = new MongoRecipeStore("localhost", 27017, "testsh", "recipes");
	 * 
	 * for (ComputeJob job : this.jobs) { Recipe r = new Recipe(job.getName()); Step s = new Step(""); BashOperation bsho = new BashOperation(); bsho.setScript(job.getComputeScript()); s.add(bsho);
	 * r.add(s);
	 * 
	 * if (0 < job.getPrevSteps_Name().size()) { for (String previous : job.getPrevSteps_Name()) { r.addDependency(previous); } }
	 * 
	 * msr.putRecipe(r); }
	 * 
	 * /* r = new Recipe("hello world2"); r.addDependency("hello world"); s = new Step("bla"); bsho = new BashOperation(); bsho.setScript("echo Hello to you!"); s.add(bsho); r.add(s);
	 * 
	 * msr.putRecipe(r);
	 */
	/* } */
