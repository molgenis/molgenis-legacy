...
public class TestJQGridPlugin {
	...
	for (final Entry<String, String> entry : map.entrySet()) {
	    when(request.getParameter(entry.getKey())).thenReturn(
		    entry.getValue());
	}
	...
	assertEquals(
		"molgenis.do?__target\u003dtest\u0026__action\u003ddownload_json",
		map2.get("url"));
	assertEquals(Arrays.asList("Code", "Name", "Continent", "Region",
		"SurfaceArea", "IndepYear", "Population", "LifeExpectancy",
		"GNP", "GNPOld", "LocalName", "GovernmentForm", "HeadOfState",
		"Capital", "Code2", "ID", "Name", "CountryCode", "District",
		"Population"), map2.get("colNames"));
	final Map<?, ?> countryCodeColumn = ((ArrayList<Map<?, ?>>) map2
		.get("colModel")).get(0);
	...
	assertEquals(Arrays.asList("eq", "ne", "bw", "bn", "ew", "en", "cn",
		"nc"),
		((Map<?, ?>) countryCodeColumn.get("searchoptions"))
			.get("sopt"));
