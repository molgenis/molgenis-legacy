...
public class EntityTable extends AbstractFilterableTupleTable {
	 ...
	 */
	public EntityTable(final Database database,
	...
	public List<Tuple> getRows() {
		try {
			...
			if (getFilters().size() > 0) {
				q.addRules(getFilters().toArray(
						new QueryRule[getFilters().size()]));
			}
		...
		try {
			if (getFilters().size() > 0)
				return getDb().count(
						entityClass,
						getFilters()
								.toArray(new QueryRule[getFilters().size()]));
			else
