...
public class JQGridPlugin extends EasyPluginController<ScreenModel> {
		...
		tupleTableBuilder = new TupleTableBuilder() {
			...
			public TupleTable create(Database db, Tuple request)
				...
				try {
					...
					final SQLQueryImpl query = new SQLQueryImpl(connection,
							dialect);
					...
					if (joinTable) {
						...
						getTableAndColumnNames(request, tableNames,
								columnNames, true);
						...
						final List<JoinQueryTable.Join> joins = Arrays
								.asList(new JoinQueryTable.Join("Country",
										"Country.Code", "City",
										"City.CountryCode"));
						return new JoinQueryTable(query, tableNames,
								columnNames, joins, db);
					}
			...
			private void getTableAndColumnNames(Tuple request,
				...
				if (request != null) {
					...
					final List<String> columns = (List<String>) new Gson()
							.fromJson((String) request.getObject("colNames"),
									Object.class);
	...
	public Show handleRequest(Database db, Tuple request, OutputStream out)
		...
		try {
			...
			// if (tupleTable instanceof FilterableTupleTable) { //is this a
			// good idea?
			...
			// addSortOrderLimitOffset(request, (QueryTable) tupleTable,
			// offset);

			renderData(((MolgenisRequest) request).getRequest(),
					((MolgenisRequest) request).getResponse(), page,
	...
	// private void addSortOrderLimitOffset(Tuple request, QueryTable
	// queryTable, int offset) {
	//
	// final int limit = request.getInt("rows");
	...
	// final LinkedHashMap<String, SimpleExpression<? extends Object>> selectMap
	// = queryTable.getSelect();
	//
	...
	// query.offset(offset);
	// }
	// final ComparableExpressionBase<?> sortColumn =
	// ((ComparableExpressionBase<?>) selectMap.get(sidx));
	// if (sortColumn != null) {
	...
	private void renderData(HttpServletRequest request,
		...
		if (StringUtils.isEmpty(strViewType)) { // strange that the grid doesn't
												// submit it in first load!
			strViewType = "JQ_GRID";
		}
		try {
			final String viewFactoryClassName = request
					.getParameter("viewFactoryClassName");
			...
			view.export(servletContext, request, response,
					request.getParameter("caption"), this, tupleTable,
					totalPages, page);
		} catch (Exception e) {
	...
	// @SuppressWarnings("unchecked")
	// private static void addFilters(final Tuple request, final QueryTable
	// queryTable) throws TableException {
	// try {
	// final SQLQuery query = queryTable.getQuery();
	// final LinkedHashMap<String, SimpleExpression<? extends Object>> selectMap
	// = queryTable.getSelect();
	...
	// final String filtersParameter = request.getString("filters");
	// if (StringUtils.isNotEmpty(filtersParameter)) {
	// final StringMap filters = (StringMap) new
	// Gson().fromJson(filtersParameter, Object.class);
	// final String groupOp = (String) filters.get("groupOp");
	//
	// final ArrayList<StringMap<String>> jsonRules =
	// (ArrayList<StringMap<String>>) filters.get("rules");
	//
	...
	// expr = expr.or(rhs);
	// } else {
	// throw new IllegalArgumentException(String.format("Unkown groupOp: %s",
	// groupOp));
	// }
	// } else {
	// expr = rhs;
	 ...
	 *            be added.
	 */
	private void addSortRules(final String sidx, final String sord,
			final List<QueryRule> rules) {
		if (StringUtils.isNotEmpty(sidx)) {
			final QueryRule sort = new QueryRule();
			sort.setValue(sidx);
			sort.setOperator(StringUtils.equals(sord, "asc") ? Operator.SORTASC
					: Operator.SORTDESC);
			rules.add(sort);
		}
	}
