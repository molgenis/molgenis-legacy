...
public class MatrixViewer extends HtmlWidget
{
	 ...
	 * 
	 */
	public MatrixViewer(ScreenController<?> callingScreenController, String name, SliceableMatrix<?, ?, ?> matrix,
			boolean showLimitControls, int selectMultiple, boolean showDownloadOptions, boolean showValueValidRange,
	...
	{
		this(callingScreenController, name, matrix, showLimitControls, selectMultiple, showDownloadOptions,
				showValueValidRange, filterRules);
	 ...
	 * 
	 */
	public MatrixViewer(ScreenController<?> callingScreenController, String name, SliceablePhenoMatrix<?, ?> matrix,
			boolean showLimitControls, int selectMultiple, boolean showDownloadOptions, boolean showValueValidRange,
	...
	public MatrixViewer(ScreenController<?> callingScreenController, String name, SliceablePhenoMatrix<?, ?> matrix,
			boolean showLimitControls, int selectMultiple, boolean showDownloadOptions, boolean showValueValidRange,
			boolean showNameFilter, List<MatrixQueryRule> filterRules, MatrixQueryRule columnRule) throws Exception
	{
		this(callingScreenController, name, matrix, showLimitControls, selectMultiple, showDownloadOptions,
				showValueValidRange, filterRules, columnRule);
		this.showNameFilter = showNameFilter;
	}
				...
				for (int rowIndex = 0; rowIndex < values.length; rowIndex++)
				{
					for (int colIndex = 0; colIndex < values[0].length; colIndex++)
					{
						...
						else if (values[rowIndex][colIndex].size() == 0)
						{
							...
							String newValue = t.getString(EditableJQueryDataTable.MATRIX_EDIT_VALUE + "_" + colIndex
									+ "_" + rowIndex);

							if (!(newValue == null || newValue.equals("NA") || newValue.trim().isEmpty()))
							{
								ObservedValue newOv = new ObservedValue();
								newOv.setFeature_Name(((ObservationElement) this.getMatrix().getColHeaders()
										.get(colIndex)).getName());
								newOv.setTarget_Name(((ObservationTarget) this.getMatrix().getRowHeaders()
						...
						else
						{
							...
							if (!oldValue.equals(newValue))
							{
								if (!(newValue == null || newValue.equals("NA") || newValue.trim().isEmpty()))
								{
									...
									// database...
									ObservedValue updateMe = db.find(ObservedValue.class,
											new QueryRule(ObservedValue.ID, Operator.EQUALS, id)).get(0);
	...
	public String toHtml()
	{
		try
		{
			...
			{
				result += "<div><p><a href=\"tmpfile/" + downloadLink + "\">Download your export (" + downloadLink
						+ ")</a></p></div>";
			}
	...
	public String renderTable() throws MatrixException
	{
						...
						if (rowValues[col] != null && rowValues[col].size() > 0)
						{
							...
							for (Observation val : rowValues[col])
							{
								...
								else
								{
									...
									dataTable.setCell(pos, row, dataTable.getCell(pos, row) + "<br />" + valueToShow);
								}
							}
						}
						else
		...
		if (selectMultiple > 1)
		{
			...
			for (int i = 0; i < row; i++)
			{ // putting for-loop inside javascript failed so we do it in this
				// ugly manner...
		...
		if (colHeaders != null && colHeaders.size() > 0 && colHeaders.get(0) instanceof Entity)
		{
			...
			if (!headers.isEmpty() && headers.get(0) instanceof Measurement && matrix instanceof SliceablePhenoMatrixMV)
			{
				...
				for (Entry<Protocol, List<Measurement>> p : measurementsByProtocol.entrySet())
				{
					...
					{
						colId.addOption(protocol.getId() + ";" + measurement.getId(),
								String.format("%s (%s)", measurement.getName(), protocol.getName()));
					}
				}
			}
			else
		...
		try
		{
			for (Object meas : matrix.getColHeaders())
			{
				if (meas instanceof ObservationElement)
				{
					...
					if (measr.getId() != null && mqr.getDimIndex() != null
							&& measr.getId().intValue() == mqr.getDimIndex().intValue())
					{
	...
	public File makeFile(String visAll, String extension)
	{
		...
		File file = new File(tmpDir.getAbsolutePath() + File.separatorChar + "Export" + visAll + "_"
				+ selectDate().replace(":", "_") + "." + extension);
		return file;
	}
	...
	@SuppressWarnings("unchecked")
	public void downloadCsv(List<?> rows, List<?> cols, List<? extends ObservedValue>[][] values, File file)
			...
			WriteException
	{
		...
		else
		{
			...
			WritableCellFormat cellFormat = new WritableCellFormat(new WritableFont(WritableFont.ARIAL, 10,
	...
	public void resetColumns(final List<Integer> columnIds) throws DatabaseException
	{
		...
		final Protocol patient = db.getEntityManager()
				...
				.setParameter("name", "patient").getSingleResult();
		...
		final Measurement paIDPk = db
				...
				.createQuery(
						...
						Measurement.class).setParameter("measurementName", "pa_id")
	...
	public void delegate(String action, Database db, Tuple request) throws HandleRequestDelegationException
	{
		...
		{
			this.callingScreenController.getModel().setMessages(new ScreenMessage("Unknown action: " + action, false));
		...
		catch (Exception e)
		{
			logger.error("call of " + this.getClass().getName() + "(name=" + this.getName() + ")." + action
					+ " failed: " + e.getMessage());
			e.printStackTrace();
			this.callingScreenController.getModel().setMessages(new ScreenMessage(e.getCause().getMessage(), false));
		}
	}
	...
	private String valueListToString(List<? extends ObservedValue> valueList)
	{
		...
		for (int i = 0; i < valueList.size(); i++)
		{
			...
			if (tmpValue == null)
			{ // null values become NA, empty ones remain empty
				tmpValue = "NA";
			}
			result += tmpValue;
		}
