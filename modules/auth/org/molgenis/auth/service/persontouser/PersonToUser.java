...
{
	private PersonToUserModel model = new PersonToUserModel();
	...
	{
		return "org/molgenis/auth/service/persontouser/PersonToUser.ftl";
	}
		...
		if (request.getString("__action") != null)
		{
			...
			try
			{
				if (action.equals("upgrade"))
				{
					...
					//copy properties except ID and __Type, plus 'active' and 'password'
					...
					if(gid != -1)
					{
						MolgenisGroup mg = db.find(MolgenisGroup.class, new QueryRule(MolgenisGroup.ID, Operator.EQUALS, gid)).get(0);
					...
					//great success
					this.setMessages(new ScreenMessage("The selected person is now a user with password 'changeme'.", true));
