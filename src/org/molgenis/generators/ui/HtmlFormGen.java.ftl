...
<#list allFields(entity) as field>
	<#assign inputtype = Name(field.getType().toString())>
		//${JavaName(field)}: ${field}
		{
		    <#if field.type == "xref" >
		    	//xref
				${inputtype}Input input = new ${inputtype}Input("${JavaName(entity)}.${name(field)}",getEntity().get${JavaName(field)}_${Name(field.getXrefField())}());
			<#else>
				${inputtype}Input input = new ${inputtype}Input("${JavaName(entity)}.${name(field)}",getEntity().get${JavaName(field)}());
			</#if>
