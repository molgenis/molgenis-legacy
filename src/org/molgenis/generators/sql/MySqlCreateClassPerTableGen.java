...
//	public void sortEntitiesByXref(List<Entity> entityList, Model model) throws Exception
//	{
//		if(true)
//			return;
//		//buble sort
...
//		{
//			if (count == maxcount)
//				throw new Exception("you have a cyclic relationship in you database scheme. check the 'swapped .. with ..' messages for clues.");
...
//			{
//				if (field.getType().toString() == "xref" || field.getType().toString() == "mref")
//				{
////					Field xref_field = model.getEntity(field.getXRefEntity()).getField(field.getXRefField());
////					if( !field.getName().equals(xref_field.getName()) && !(field.getEntity().isAncestor(xref_field.getXRefEntity()) || field.getEntity().getImplements().contains(xref_field.getXRefEntity())))
...
//					logger.debug("swapped " + entityList.get(xrefPosition).getName() + " with " + entityList.get(i).getName());
