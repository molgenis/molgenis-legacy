...
<#list allFields(entity) as f>
	<#if f.type == 'xref' && f.xrefLabelNames[0] != f.xrefFieldName>
		<#if f.xrefLabelNames?size &gt; 1>
			//create xref rule filtering ${f.xrefEntityName} on the combination of labels ${csv(f.xrefLabelNames)}
			{
		...
		<#else>
			//create xref rule filtering ${f.xrefEntityName} on the label ${csv(f.xrefLabelNames)}
			{
