...
public class MolgenisModelValidator {
    ...
    public static void validate(Model model, MolgenisOptions options)
        ...
        validateMrefDuplicateNames(model);
...
//		}
    ...
    public static void validatePrimaryKeys(Model model)
        ...
        for (Entity e : model.getEntities()) {
            ...
            if (!e.isAbstract()) {
                if(e.getKeyFields(0).size() > 1) {
                    throw new MolgenisModelException(String.format("Entity: %s needs a primary key of one field! Pk of multiple columns not supported!", e.getName()));
                }    
                ...
                try {
                    ...
                    if (mrefEntity == null) {
                        ...
                        field.setXRefVariables(xref_entity_to.getName(),
                                xref_field_to.getName(), xref_field_from.getXrefLabelNames());
                        ...
                        for (Field key : xref_entity_from.getKeyFields(Entity.PRIMARY_KEY)) {
                            field = new Field(mrefEntity,
                                    new XrefField(), xref_field_from.getMrefLocalid(), null, false,
                                    false, false, null);
    ...
    public static void validateViews(Model model) throws MolgenisModelException {
        ...
        for (View view : model.getViews()) {
            ...
            for (String viewentity : view.getEntities()) {
                ...
                if (entity == null) {
                    throw new MolgenisModelException("Entity '"
            ...
            for (Pair<Entity, Entity> p : references) {
                if (!viewentities.contains(p.getA())) {
                    viewentities.add((Entity) p.getA());
                }
                if (!viewentities.contains(p.getB())) {
                    viewentities.add((Entity) p.getB());
                }
            }
                ...
                if (field.getType() instanceof XrefField
                        ...
                        if (xref_label == null) {
                            ...
                            for (String validLabel : candidates.keySet()) {
                                ...
                                if (xref_label_name.equals(validLabel)) {
                                    xref_label = candidates.get(validLabel).get(
                                            candidates.get(validLabel).size() - 1);
                                }
                    ...
                    for (Unique unique : xref_entity.getAllKeys()) {
                        for (Field keyfield : unique.getFields()) {
                            if (keyfield.getName().equals(xref_field_name)) {
                                isunique = true;
                            }
                        }
                    }
            ...
            if (entity.isRootAncestor()) {
                ...
                if (!entity.isAbstract()) {
                    ...
                    rootAncestor = new Entity("_" + entity.getName()
                            + "Interface", entity.getName(), model.getDatabase());
                    ...
                    for (Field f : keyfields) {
                        Field key_field = new Field(rootAncestor, f.getType(),
                                f.getName(), f.getName(), f.isAuto(), f.isNillable(), f.isReadOnly(), f.getDefaultValue());
    ...
    public static void validateNamesAndReservedWords(Model model,
        ...
        if (options.db_driver.contains("oracle")) {
            keywords.addAll(fillOraclekeywords(options));
        }
        ...
        for (Entity e : model.getEntities()) {
            ...
            for (Field f : e.getFields()) {
                ...
                if (f.getType() instanceof XrefField
                    ...
                    if (xref_entity != null
                            && (keywords.contains(xref_entity.toUpperCase()) || keywords.contains(xref_entity.toLowerCase()))) {
    ...
    private static List<String> fillOraclekeywords(MolgenisOptions options) {
        List<String> keywords = new ArrayList<String>();

        try {
            Class.forName(options.db_driver);
            Connection conn = DriverManager.getConnection(options.db_uri, options.db_user, options.db_password);

            conn.setAutoCommit(false);
            Statement stmt = conn.createStatement();

            ResultSet rset = stmt.executeQuery("SELECT keyword FROM sys.v_$reserved_words WHERE reserved = 'Y' OR res_attr = 'Y' OR res_semi = 'Y'");
            while (rset.next()) {
...
//            keywords.remove("BATCH");
    ...
    public static final String[] HSQL_KEYWORDS = {"ALIAS", "ALTER", "AUTOCOMMIT", "CALL", "CHECKPOINT", "COMMIT",
        "CONNECT", "CREATE", "COLLATION", "COUNT", "DATABASE", "DEFRAG",
        "DELAY", "DELETE", "DISCONNECT", "DROP", "END", "EXPLAIN", "EXTRACT",
     ...
     */
    private static final String[] MYSQL_KEYWORDS = {"Type", "ADD", "ALL", "ALTER", "ANALYZE", "AND", "AS", "ASC",
     ...
     */
    protected static final String[] JAVA_KEYWORDS = {"abstract", "continue", "for", "new", "switch", "assert", "default",
    ...
    protected static final String[] JAVASCRIPT_KEYWORDS = {"function"};
    ...
    private static void validateMrefDuplicateNames(Model model) throws MolgenisModelException {
        //Pair<entity.name, relation.name>, field of other side of relation
        Map<Pair<String, String>, List<String>> map = new TreeMap<Pair<String, String>, List<String>>(
                new Comparator<Pair<String, String>>() {

                    @Override
                    public int compare(Pair<String, String> t, Pair<String, String> t1) {
                        int value = t.getA().compareToIgnoreCase(t1.getA());
                        if (value != 0) {
                            return value;
                        }

                        return t.getB().compareToIgnoreCase(t1.getB());
                    }
                });
        ...
        for (Entity e : model.getEntities()) {
            for (Field f : e.getImplementedFields()) {
                if (f.isMRef()) {
                    Pair<String, String> key = new Pair<String, String>(f.getXrefEntityName(), f.getName());
                    if (map.containsKey(key)) {
                        map.get(key).add(e.getName());
                    } else {
                        List<String> relations = new ArrayList<String>();
                        relations.add(e.getName());
                        map.put(key, relations);
                    }
                }
            }
        }
        ...
        for (Entry<Pair<String, String>, List<String>> entry : map.entrySet()) {
            if (entry.getValue().size() > 1) {
                String errorMessage = String.format("Name clash in Entities: %s for field: %s", StringUtils.join(entry.getValue(), ","), entry.getKey().getB());
                exceptionMessage += errorMessage + "\n";
                logger.error(errorMessage);
            }
        }
        if (StringUtils.isNotEmpty(exceptionMessage)) {
            throw new MolgenisModelException(exceptionMessage);
        }
    }
}
