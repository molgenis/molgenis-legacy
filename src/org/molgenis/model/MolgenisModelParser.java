...
public class MolgenisModelParser {
    ...
    public static void parseMatrix(Model model, Element element)
            throws MolgenisModelException {
        // check parameters
        String[] keywords = new String[]{"name", "content_entity", "content", "container", "row", "col",
    ...
    public static void parseField(Entity entity, Element element)
            throws MolgenisModelException {
        // check for illegal words
        String[] keywords = new String[]{"type", "name", "label", "auto", "nillable", "optional", "readonly",
        ...
        if (xref_label.equals("") || xref_label == null) {
            xref_label = null;
        }
        ...
        if (hidden.equals("true") && !nillable.equals("true")
                && (default_value.equals("") && !auto.equals("true"))) {
            throw new MolgenisModelException(
                    "field '"
                    + name
                    + "' of entity '"
        ...
        if (filter.equals("true")) {
            ...
            if (filtervalue == "") {
                logger.warn("no value specified for filter in field '" + name
                        + "' of entity '" + entity.getName() + "'");
            }
        ...
        if (type.equals("string")) {
            ...
            if (mref_name.equals("") && (xref_entity.equals(""))) {
                throw new MolgenisModelException(
            ...
            if (type.equals("mref")) {
                ...
                if (!mref_localid.equals("")) {
                    field.setMrefLocalid(mref_localid);
                }
                if (!mref_remoteid.equals("")) {
    ...
    public static void parseParameter(Method method, Element element)
        ...
        // construct
        Parameter parameter = new Parameter(method, Parameter.Type.getType(element.getAttribute("type")), element.getAttribute("name"), element.getAttribute("label"), false,
    ...
    public static void parseQueryRule(Model model, MethodQuery query,
        ...
        query.addRule(new MethodQuery.Rule(element.getAttribute("field"),
                element.getAttribute("operator"), element.getAttribute("parameter")));
    }
            ...
            if (element.getTagName().equals("menu")) {
                ...
                if (filter != null && filter.equals("true")) {
                    ...
                    if (element.getAttribute("filtervalue") == "") {
                        logger.warn("filtervalue is missing for subform of screen '"
                                + parent.getName() + "'");
                    }
                    form.setFilter(true);
                ...
                // check required properties
                if (element.getAttribute("parentfield") == "") {
                    throw new MolgenisModelException(
                            "parentfield is missing for tree screen '" + name + "'");
                }
                if (element.getAttribute("idfield") == "") {
                    throw new MolgenisModelException(
                            "idfield is missing for tree screen '" + name + "'");
                }
                if (element.getAttribute("labelfield") == "") {
                    throw new MolgenisModelException(
                            "labelfield is missing for tree screen '" + name + "'");
                }

                Tree tree = new Tree(name, parent, element.getAttribute("parentfield"), element.getAttribute("idfield"), element.getAttribute("labelfield"));
                tree.setLabel(label);
                tree.setGroup(group);
