...
public class FormView  implements TwoStepView<Form>
{
	...
	public String render(Form element, Theme theme) throws RenderException
	{
		...
		switch(element.getType())
		{
			...
			case INLINE:
				return String.format("<form class=\"well form-inline\">%s%s</form>", legend, renderDefault(element, theme));
			case HORIZONTAL:
				return String.format("<form class=\"well form-horizontal\">%s<fieldset>%s</fieldset></form>", legend, renderHorizontal(element, theme));	
			default:
	...
	private String renderHorizontal(Form element, Theme theme) throws RenderException
	{
		...
		for(MolgenisComponent<?> c: element.getComponents())
		{
			if(c instanceof HtmlInput)
			{
				...
				result += String.format("<div class=\"control-group\"><label class=\"control-label\" for=\"%s\">%s</label><div class=\"controls\">%s</div></div>",c.getId(),label,theme.render(c));
