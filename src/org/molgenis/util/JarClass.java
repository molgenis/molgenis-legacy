...
public class JarClass
{
			...
			throws Exception
	{
		...
		try
		{
			...
			do
			{
				...
				{
					throw new Exception(
							"Unable to get next jar entry from jar file '"
									+ jar + "'");
				}
	...
	public static ArrayList<String> getClassesFromJARFile(String jar,
	...
	private static void extractClassFromJar(final String jar,
			final String packageName, final ArrayList<String> classes,
			JarEntry jarEntry) throws Exception
	{
		...
		if (className.endsWith(".class"))
		{
			...
			if (className.startsWith(packageName))
			{
				...
				{
					throw new Exception("unable to find class named "
							+ className.replace('/', '.') + "' within jar '"
							+ jar + "'");
				}
			}
		}
