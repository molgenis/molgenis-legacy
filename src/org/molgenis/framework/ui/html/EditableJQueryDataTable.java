...
public class EditableJQueryDataTable extends JQueryDataTable
{
	...
	String printRow(String row, int rowCount) {
		...
		for (int colCount = 0; colCount < cols.size(); colCount++) {
			...
			(
				getCellString(colCount, rowCount).contains("<")
				?
				getCellString(colCount, rowCount)
				: 
				"<input name=\""+MATRIX_EDIT_VALUE+"_"+colCount+"_"+rowCount+"\" type=\"text\" value=\""+HtmlUtils.htmlEscape(getCellString(colCount, rowCount))+"\"/>"
			)
			+ "</td>");
		}
		result += "</tr>";
