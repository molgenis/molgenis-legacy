...
<body>
	...
	<p align="right">
		<img class="edit_button" src="generated-res/img/save.png" alt="Save" onclick="if( validateForm(${screen.name}_new,new Array(${required})) ) { if( window.opener.name == '' ){ window.opener.name = 'molgenis'+Math.random();} document.forms.${screen.name}_new.target = window.opener.name; document.forms.${screen.name}_new.submit(); window.close();}" title="save new record"/>
		<img class="edit_button" src="generated-res/img/cancel.png" alt="Cancel" onclick="window.close();" title="cancel adding a new record"/>
	</p>
</body>
</#macro>
<#--
...
<body>
	...
	<#list screen.getNewRecordForm() as input>
			...
			<tr>
				<td></td>
				<td >					
					<img class="edit_button" src="generated-res/img/save.png" alt="Add filter" onclick=" document.forms.${screen.name}_filter.submit(); window.opener.location.href = window.opener.location.href; window.close();"/>
					<img class="edit_button" src="generated-res/img/cancel.png" alt="Cancel" onclick="window.close();" title="cancel adding a new filter"/>				
				</td>
			</tr>
		</table>
	</form>
...
<body onload="<#list screen.getNewRecordForm().inputs as input><#if !input.isHidden()>${screen.name}_massupdate.${input.name}.disabled = true; </#if></#list>">
	...
	<p align="right">
		<input type="image" src="generated-res/img/cancel.png" alt="Cancel" name="action_cancel" value="Cancel" onclick="window.close();" title="cancel adding a new record"/>
	</p>
	</#if>
</body>
</#macro>
	...
	<tr>
		...
		<th><label>&nbsp;</label></th>
			...
			<#list record.inputs as input>				
				...
				<#else>
					<#if input.isHidden()>
			<#--<img src="generated-res/img/open.png" title="show ${input.getLabel()}" onclick="setInput('${screen.name}_form','_self','','${screen.name}','showColumn','iframe'); document.forms.${screen.name}_form.attribute.value ='${input.getName()}'; document.forms.${screen.name}_form.submit();" />-->
					<#else>
			<#--<img src="generated-res/img/close.png" title="hide ${input.getLabel()}" onclick="setInput('${screen.name}_form','_self','','${screen.name}','hideColumn','iframe'); document.forms.${screen.name}_form.attribute.value='${input.getName()}'; document.forms.${screen.name}_form.submit();" />-->
			<th><label class="tableheader" onclick="setInput('${screen.name}_form','_self','','${screen.name}','sort','iframe'); document.forms.${screen.name}_form.__sortattribute.value='${input.getName()}'; document.forms.${screen.name}_form.submit();">
					${input.getLabel()}<#if screen.getSort() == input.getName()> <#if screen.getSortMode().toString() == "SORTASC"><img src="generated-res/img/sort_asc.gif"><#else><img src="generated-res/img/sort_desc.gif"></#if></#if>
			</label>
					</th>
					</#if>
				</#if>
...
<tr class="form_listrow${rowcolor}">
	<td>
		<label>${offset?c}. 
		<#if readonly == "*" >
			<img class="edit_button" src="generated-res/img/recordview.png" title="view record" alt="edit${offset?c}" onClick="setInput('${screen.name}_form','_self','','${screen.name}','editview','iframe'); document.forms.${screen.name}_form.__offset.value='${offset?c}'; document.forms.${screen.name}_form.submit();">${readonly}</label>
		<#else>
			<img class="edit_button" src="generated-res/img/editview.gif" title="edit record" alt="edit${offset?c}" onClick="setInput('${screen.name}_form','_self','','${screen.name}','editview','iframe'); document.forms.${screen.name}_form.__offset.value='${offset?c}'; document.forms.${screen.name}_form.submit();">${readonly}</label>
		</#if>
	</td>
		
		<td><input type="checkbox" name="massUpdate<#if record.entity.readonly>_readonly</#if>" value="${record.entity.idValue?c}"></td>
		<#list record.inputs as input>
				<#if input.isHidden()>
				<#else>
					<#if input.getTarget() != "" && input.getObject()?exists >
...
<body>
	...
	<form action="" method="post" enctype="multipart/form-data" name="${screen.name}_upload">
		...
		<!--<img class="edit_button" src="generated-res/img/save.png" alt="Save" onclick="document.forms.${screen.name}_upload.submit(); window.opener.location.href = window.opener.location.href; window.close();" title="upload csv"/>-->
		<img class="edit_button" src="generated-res/img/save.png" alt="Save" onclick="if( window.opener.name == '' ){ window.opener.name = 'molgenis'+Math.random();} document.forms.${screen.name}_upload.target = window.opener.name; document.forms.${screen.name}_upload.submit(); window.close();" title="upload csv"/>
		<img class="edit_button" src="generated-res/img/cancel.png" alt="Cancel" onclick="window.close();" title="cancel upload csv"/>
		</td></tr>
		</table>
	</form>
</body>
