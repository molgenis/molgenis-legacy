	...
	<#list screen.getMenus() as menu>
		...
		<#list menu.getCommands() as command>
			<#if command.isVisible()>
										<tr><td class="submenuitem" onclick="${command.getJavaScriptAction()}"><img src="${command.icon}" align="left" />${command.label}</td></tr>
			</#if>
		</#list>
									</table>
								</div>											
...
<table>
	...
	<tr title="${input.getDescription()}">
		...
		<td>${input.toHtml()}<#if !input.isNillable() && !input.isReadonly()>*</#if></td>
			...
			<#if !input.isNillable() && !input.isHidden() && !input.isReadonly()>
				<#if requiredcount &gt; 0><#assign required = required + "," /></#if>
				<#assign required = required + input.id />
				<#assign requiredcount = requiredcount + 1 />
			</#if>
		</#list>
...
<form name="${screen.name}_form"  id="${screen.name}_form" target="" action="" method="post" style="margin: 0px;" enctype="multipart/form-data">
...
var panel = new YAHOO.widget.Panel('${screen.name}', {
    draggable: false,
    close: false,
    underlay: 'none',
    autofillheight: "body", // default value, specified here to highlight its use in the example            
});
panel.setHeader('${screen.label}');
panel.render();
</script>
