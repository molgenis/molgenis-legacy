...
public abstract class JDBCConnectionHelper extends AbstractDatabase
{
	 ...
	 * @throws DatabaseException
	 */
	public static String createWhereSql(JDBCMapper<?> mapper, boolean isNested,
			boolean withOffset, QueryRule... rules) throws DatabaseException
	{
		...
		if (rules != null)
		{
			for (QueryRule r : rules)
			{
				...
				else
				{
					...
					// String tablePrefix = "";
					if (mapper != null) rule.setField(mapper
					...
					else if (rule.getOperator() == QueryRule.Operator.NESTED)
					{
						...
						if (nestedrules.length > 0)
						{
							...
							where_clause.append("(");
							where_clause.append(createWhereSql(mapper, true,
									false, nestedrules));
							where_clause.append(")");
						}
					}
		...
		return result + createSortSql(mapper, rules)
				+ createLimitSql(withOffset, rules);
	}

	/** Helper method for creating a sort clause */
	private static String createSortSql(JDBCMapper<?> mapper,
			QueryRule... rules)
	{
	 ...
	 */
	public static String createSortSql(JDBCMapper<?> mapper,
			boolean reverseSorting, QueryRule rules[])
	{
		...
		if (rules != null)
		{
			...
			for (QueryRule r : rules)
			{
				...
				{
					if (mapper != null) rule.setValue(mapper
							.getTableFieldName(rule.getValue().toString()));
