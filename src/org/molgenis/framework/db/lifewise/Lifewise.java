...
public class Lifewise implements Database
{
	...
	public static String marshal(Object objectToMarshall) throws Exception
	{
		...
		if (objectToMarshall.getClass().equals(SendList.class))
		{
			classes = new Class[]
			{ SendList.class,
					((SendList) objectToMarshall).getList().get(0).getClass() };
		}
		else
		{
			classes = new Class[]
			{ objectToMarshall.getClass() };
		}
	...
	public static Object unMarshal(String xmlObjects, Class[] klass)
			throws Exception
	{
	...
	public static Object unMarshal(String xmlObjects, Class klass)
			throws Exception
	{
		return unMarshal(xmlObjects, new Class[]
		{ klass });
	}

	private <E extends Entity> List<E> executeClientListOperation(
			Class<E> klazz, String operation, List<QueryRule> qr)
	{
		try
		{
			...
			{ SendList.class, klazz };
			Object o = unMarshal(xmlResult, classes);
			o.toString();

			SendList<E> result = (SendList<E>) unMarshal(xmlResult, new Class[]
			{ SendList.class, klazz });
		...
		catch (Exception ex)
		{
			ex.printStackTrace();
			logger
					.error("int executeClientOperation(String operation, List<E> entities)");
		}
		return null;
	}

	private <E extends Entity> int executeClientOperation(String operation,
			List<E> entities)
	{
	...
	private <E extends Entity> int executeClientOperation(String operation,
			List<E> entities, boolean updateEntities)
	{
		try
		{
			if (entities.size() > 0)
			{
				...
				String xmlResult = (String) client.execute(operation,
						new Object[]
						{ xmlMessage });
				@SuppressWarnings("unchecked")
				SendList<E> result = (SendList<E>) unMarshal(xmlResult,
						sendList.getClass());
				if (updateEntities)
				{
					int idx = 0;
			...
			QueryRule... rules) throws DatabaseException
	{
		try
		{
			...
			String xmlResult = (String) client.execute("QueryHandler.count",
					params);
			QueryResult result = ((QueryResult) unMarshal(xmlResult,
					QueryResult.class));
		...
		{
			logger
					.error("count(Class<E> entityClass, QueryRule... rules) throws DatabaseException failed"
							+ ex.toString());
			ex.printStackTrace();
			return -1;
		}
			...
			throws DatabaseException
	{
		try
		{
			String idField = mappers.get(entityClass.getSimpleName()).create()
					.getIdField();
			QueryRule qr = new QueryRule(idField, Operator.EQUALS, id);

			String xmlQueryRule = marshal(qr);
			String xmlResult = (String) client.execute("QueryHandler.findById",
					new Object[]
					{ xmlQueryRule });
		...
		catch (Exception ex)
		{
			ex.printStackTrace();
			logger
					.error("E findById(Class<E> entityClass, Object id) throws DatabaseException failed");
			return null;
		}
	}
