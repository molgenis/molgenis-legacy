...
public class FinalReportToTriTyper {
    ...
    /**
     * Starts parsing a genotype report file, which can be in different formats. This import program can accomodate many formats, and uses buffering to achieve sufficient performance.
     */
    public FinalReportToTriTyper(String inputFile, String outputDirString, boolean isIlluminaFinalReportFile, String delimiter, String decimalSeparator) {
        ...
        //Vectors and hashes for determining file size of final report file:
        HashMap hashInd = new HashMap();
        Vector vecInd = new Vector();
        HashMap hashSNP = new HashMap();
        Vector vecSNP = new Vector();
        ...
        try {
            ...
            if (isIlluminaFinalReportFile) {
                ...
                while ((str = in.readLine()) != null) {
                    ...
                    if (data[0].trim().equals("[Data]")) break;
            ...
            if (data.length<=1) {
                ...
                if (delimiter.equals(" ")) delimiterDescription = "space";
                if (delimiter.equals(",")) delimiterDescription = "comma";
                if (delimiter.equals(";")) delimiterDescription = "semicolon";
            ...
            for (int d=0; d<data.length; d++) {
                ...
                if (column.equals("sample id")) columnSample = d;
                if (column.equals("snp name")) columnSNP = d;
                if (column.contains("allele1")) columnAllele1 = d;
                if (column.contains("allele 1")) columnAllele1 = d;
                if (column.contains("allele2")) columnAllele2 = d;
                if (column.contains("allele 2")) columnAllele2 = d;
                if (column.equals("r")) columnR = d;
                if (column.equals("theta")) columnTheta = d;
            }
            if (columnSample==-1) {
            ...
            for (int d=0; d<data.length; d++) {
                String column = data[d].trim().toLowerCase();
                if (column.equals("x")) columnX = d;
                if (column.equals("y")) columnY = d;
            }
		    ...
		    // System.out.println(str);
                    ...
                    linesProcessed++;
                    if (linesProcessed%500000==0) System.out.println(linesProcessed + "\tLines processed. Number of unique SNPs read so far:\t" + vecSNP.size() + "\tNumber of unique Individuals read so far:\t" + vecInd.size());
                }
                ...
                File fileSNPMappings = new File(outputDirString + "SNPMappings.txt");
                if (!fileSNPMappings.canRead()) {
                    ...
                    try {
                        System.out.println("\nLoading SNP mappings from file:\t" + fileSNPMappings.getAbsolutePath());
                        BufferedReader inSNP = new BufferedReader(new FileReader(fileSNPMappings));
                        String str2;
                        ...
                        while ((str2 = inSNP.readLine()) != null) {
                            ...
                            if (hashSNP.containsKey(data[2])) {
                                if (data[1].length()!=9) {
                                    needsSorting = true;
                                    while (data[1].length()<9) data[1] = "0" + data[1];
                                }
                        ...
                        HashMap hashSNPMappings = new HashMap();
                        Vector vecSNPMappings = new Vector();
                        ...
                        Vector vecSNPCopy = new Vector();
                        for (int snp=0; snp<vecSNP.size(); snp++) {
                            String rsName = (String) vecSNP.get(snp);
                            if (hashSNPMappings.containsKey(rsName)) snpMappingsUsed[((Integer) hashSNPMappings.get(rsName)).intValue()] = true;
                        }
                        Vector vecSNPNew = new Vector();
                        HashMap hashSNPNew = new HashMap();
                        ...
                        Vector snpsWithoutMapping = new Vector();
                        for (int snp=0; snp<vecSNP.size(); snp++) {
                            String rsName = (String) vecSNP.get(snp);
                        ...
                        System.out.println("Error while processing SNP Mappings file!");
                        e.printStackTrace();
                        System.exit(-1);
                    }
                }
                ...
                try {
                    ...
                    BufferedWriter outInd = new BufferedWriter(new FileWriter(outputDirString + "Individuals.txt"));
                    ...
                    System.out.println("Error while writing individuals to file!");
                    e.printStackTrace();
                    System.exit(-1);
                }
                ...
                try {
                    ...
                    BufferedWriter outSNP = new BufferedWriter(new FileWriter(outputDirString + "SNPs.txt"));
                    for (int snp=0; snp<vecSNP.size(); snp++) {
                ...
                hashInd.clear();
                try {
                    java.io.BufferedReader inInd = new java.io.BufferedReader(new java.io.FileReader(new File(outputDirString + "Individuals.txt")));
                    ...
                    System.out.println("Error while reading individuals from file!");
                    e.printStackTrace();
                    System.exit(-1);
                }
                ...
                hashSNP.clear();
                try {
                    java.io.BufferedReader inSNP = new java.io.BufferedReader(new java.io.FileReader(new File(outputDirString + "SNPs.txt")));
                    ...
                    System.out.println("Error while reading SNPs from file!");
                    e.printStackTrace();
                    System.exit(-1);
                }
            ...
            System.out.println("Error while processing raw data report file!");
            e.printStackTrace();
        }
        ...
        try {
            ...
            //Try to open the input file:
            File fileGenotypeFile = new File(inputFile);
            BufferedReader in = new BufferedReader(new FileReader(fileGenotypeFile));
        ...
        try {
            ...
            if (rawDataAvailable) fileRawData = new RandomAccessFile(outputDirString + "RawDataMatrix.dat", "rw");
            System.out.println("OK");
            ...
            //Set size of files:
            file.setLength(0);
            if (rawDataAvailable) fileRawData.setLength(0);

            System.out.print("Making binary files zero:\t");
            //Quickly fill using buffers:
            file.seek(0);
            if (rawDataAvailable) fileRawData.seek(0);
            byte[] emptyString = new byte[10000]; for (int s=0; s<10000; s++) emptyString[s] = 0;
            ...
            while ((str = in.readLine()) != null) {
                ...
                if (rawDataAvailable) {
                    if (columnR!=-1 && columnTheta!=-1) {
                        ...
                        //r = Math.sqrt(x * x + y * y);
                        r = x + y;
                        theta = 1; if (x>0) theta = twoDividedByPI * Math.atan2(y, x);
                    }
                ...
                if (allele1==45) allele1 = 0;
                if (allele2==45) allele2 = 0;
                if (allele1==66) {
                    allele1 = 67;
                ...
                if (fileOrderPerSampleAllSNPs || fileOrderPerSNPAllSamples) {
                    if (fileOrderPerSampleAllSNPs) {
                        if (indID!=previousIndID && previousIndID!=-1) bufferCurrentPos++;
            ...
            in.close();
            file.close();
            if (rawDataAvailable) fileRawData.close();
...
//        args[1] = "/Data/GeneticalGenomicsDatasets/RotterdamStudy/GenotypeFinalReports/TriTyper-660/";
        ...
        String outputDir = args[args.length-1].trim(); if (!outputDir.endsWith("/")) outputDir+="/";
