...
public class TriTyperToPedAndMapConverter {
    ...
    public void exportSubsetOfSNPs(String datadir, String outputDir, String snpSubsetFile) throws IOException, Exception {
	...
	exportSetOfSNPsToPedFile(snppassesQC, snpids, dataGenotypeDataset, pedFile, log);
	pedFile.close();
    ...
    public void exportAllSNPs(String datadir, String outputDir) throws IOException, Exception {
	TriTyperGenotypeData dataGenotypeDataset = new TriTyperGenotypeData();
	dataGenotypeDataset.load(datadir);
	String[] snps = dataGenotypeDataset.getSNPs();
	int numsnps = snps.length;
    ...
    private void exportSetOfSNPsToPedFile(boolean[] snppassesQC, ArrayList<Integer> snpids, TriTyperGenotypeData dataGenotypeDataset, TextFile pedFile, TextFile log) throws IOException {
	...
	while (indsprocessed < numDatasetIndividuals) {
		...
		if (dataGenotypeDataset.getIsIncluded()[i]) {
			...
			if (snppassesQC[s]) {
			    ...
			    sb.append(" ");
			    sb.append(convbyte(value1));
			    sb.append(" ");
			    sb.append(convbyte(value2));
			}
		    }
    ...
    public String convbyte(byte value1) {
