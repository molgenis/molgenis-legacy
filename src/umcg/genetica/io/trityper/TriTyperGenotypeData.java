            ...
            if(lineElems.length > 2){
                ...
                if(snpNum != null && !"null".equals(lineElems[0]) && !"null".equals(lineElems[1])){
                    ...
                    if(unknownchr || unknownchrpos){
                        nrWoAnnotation++;
//                        System.out.println("Warning: "+snpStr+" has no annotation");
                    }
                }
...
//        BufferedRandomAccessFile genotypeHandle = new BufferedRandomAccessFile(genotypeFileName, "r", individuals.length*25);
//        if(Gpio.exists(dosageFileName)){
//            System.out.println("Dosage information is available at: "+dosageFileName);
//            dosageHandle = new BufferedRandomAccessFile(dosageFileName, "r", individuals.length*25);
//        }
    ...
    private void checkFileSizes() throws IOException {
        if(Gpio.exists(genotypeFileName)){
            ...
            if(expectedfilesize != detectedsize){
                throw new IOException("Size of GenotypeMatrix.dat does not match size defined by Indivuals.txt and SNPs.txt. Expected size: "+expectedfilesize+"\tDetected size: "+detectedsize+"\tDiff: "+Math.abs(expectedfilesize-detectedsize));
            }
        } else {
        ...
        if(Gpio.exists(dosageFileName)){
            ...
            if(expectedfilesize != detectedsize){
                throw new IOException("Size of ImputedDosageMatrix.dat does not match size defined by Indivuals.txt and SNPs.txt. Expected size: "+expectedfilesize+"\tDetected size: "+detectedsize+"\tDiff: "+Math.abs(expectedfilesize-detectedsize));
            }
        }
    }
