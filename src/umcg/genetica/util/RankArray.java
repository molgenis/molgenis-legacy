...
//    }
    ...
    public RankArray() {
        swapperdouble = new cern.colt.Swapper() {
           public void swap(int a, int b) {
              double t1; int t2;
              t1 = xdouble[a]; xdouble[a] = xdouble[b]; xdouble[b] = t1;
              t2 = ydouble[a]; ydouble[a] = ydouble[b]; ydouble[b] = t2;
           }
        };
    ...
    public double[] rank(double[] x) {
        ...
        ydouble = new int[x.length]; for (int v=0; v<x.length; v++) ydouble[v] = v;
        cern.colt.GenericSorting.quickSort(0, x.length, compdouble, swapperdouble);
        double[] rank = new double[x.length]; for (int v=0; v<x.length; v++) rank[ydouble[v]] = v;
        return rank;
    }
    
    public float[] rank(float[] x) {
        this.x = x.clone();
        y = new int[x.length]; for (int v=0; v<x.length; v++) y[v] = v;
        cern.colt.GenericSorting.quickSort(0, x.length, comp, swapper);
        float[] rank = new float[x.length]; for (int v=0; v<x.length; v++) rank[y[v]] = v;
        return rank;
    }
}
