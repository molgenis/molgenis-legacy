...
(function($) {
	...
	_oExt.oStdClasses = {
		...
		"sScrollX": "dataTables_scrollx",
		"sScrollY": "dataTables_scrolly"
	};
	...
	$.fn.dataTable = function( oInit )
	{
		...
		function _fnGatherData( oSettings )
		{
			...
			{
				_fnLog( oSettings, 0, "Unexpected number of TD elements. Expected "+
		...
		function _fnDraw( oSettings )
		{
			...
			{
				oSettings.fnFooterCallback.call( oSettings.oInstance, $('tfoot:eq(0)>tr', oSettings.nTable)[0], 
					_fnGetDataMaster( oSettings ), oSettings._iDisplayStart, oSettings.fnDisplayEnd(),
					oSettings.aiDisplay );
			}
		 ...
		 * Inputs:   object:oSettings - dataTables settings object
		 * Notes:    The DOM for the scrolling is reasonably complicated here. What we do is one 
		 *   div which wraps everything else, with two children, the x scroller (which is the header)
		 *   and the y scroller (which contains the table body). Both are created when scrolling is
		 *   enabled, but only certain aspects are set depending on the type of scrolling.
		 */
		function _fnFeatureHtmlTable ( oSettings )
		{
			/* Chack if scrolling is enabled or not - if not then leave the DOM unaltered */
			if ( oSettings.oFeatures.sScrollX === "" && oSettings.oFeatures.sScrollY === "" )
			{
				return oSettings.nTable;
			}
			
			var oClasses = (typeof oInit.bJQueryUI != 'undefined' && oInit.bJQueryUI) ?
			 	_oExt.oJUIClasses : _oExt.oStdClasses;
			
			/* DIV to wrap everything up into */
			var nScroller = document.createElement('div');
			nScroller.className = oClasses.sScrollWrapper;
			
			/* Scrolling in the X (horizontal) direction - used for the header */
			var nScrollX = document.createElement('div');
			nScrollX.style.overflow = "hidden";
			nScrollX.className = oClasses.sScrollX;
			nScroller.appendChild( nScrollX );
			
			/* Scrolling in the Y (vertical) direction - used for the body */
			var nScrollY = document.createElement('div');
			nScrollY.style.overflow = "auto";
			nScrollY.className = oClasses.sScrollY;
			nScroller.appendChild( nScrollY );
			nScrollY.appendChild( oSettings.nTable );
			
			/* Move the real header (the original) into the xscrolling element */
			var nThead = $('thead', oSettings.nTable)[0];
			nScrollX.appendChild( oSettings.nTable.cloneNode(false) );
			nScrollX.getElementsByTagName('table')[0].appendChild( nThead );
			
			/* Clone the true header and we insert the clone to where we just removed the original.
			 * The relational behind doing this is to provide a 'hidden' table header which is used to
			 * set the width of the body columns
			 */
			var nClonedThead = nThead.cloneNode(true);
			oSettings.nTable.insertBefore( nClonedThead, oSettings.nTable.childNodes[0] );
			
			var nThs = nThead.getElementsByTagName('th');
			var nClonedThs = nClonedThead.getElementsByTagName('th');
			
			/* We want the hidden header to have zero height, so remove padding and borders */
			var oStyle;
			for ( var i=0, iLen=nThs.length ; i<iLen ; i++ )
			{
				oStyle = nClonedThs[i].style;
				oStyle.paddingTop = "0";
				oStyle.paddingBottom = "0";
				oStyle.borderTop = "0";
				oStyle.borderBottom = "0";
				nClonedThs[i].innerHTML = "";
			}
			
			/* When xscrolling add the width and a scroller to move the header with the body */
			if ( oSettings.oFeatures.sScrollX !== "" )
			{
				nScrollX.style.width = parseInt(oSettings.oFeatures.sScrollX,10)==oSettings.oFeatures.sScrollX ?
					oSettings.oFeatures.sScrollX+"px" : oSettings.oFeatures.sScrollX;
				
				nScrollY.style.width = parseInt(oSettings.oFeatures.sScrollX,10)==oSettings.oFeatures.sScrollX ?
					oSettings.oFeatures.sScrollX+"px" : oSettings.oFeatures.sScrollX;
				
				$(nScrollY).scroll( function (e) {
					nScrollX.scrollLeft = this.scrollLeft;
				} );
			}
			
			/* When yscrolling, add the height */
			if ( oSettings.oFeatures.sScrollY !== "" )
			{
				nScrollY.style.height = parseInt(oSettings.oFeatures.sScrollY,10)==oSettings.oFeatures.sScrollY ?
					oSettings.oFeatures.sScrollY+"px" : oSettings.oFeatures.sScrollY;
				
				/* We need a bit of space over the top of the vertial scroll bar in the header. The way
				 * to do this corss platform is using the last TH's right hand border. Go figure
				 */
				nThs[ nThs.length-1 ].style.borderRight = "18px solid transparent";
			}
			
			/* On each draw we'll set the width of the header elements or all hell breaks loose with
			 * alignment
			 */
			oSettings.aoDrawCallback.push( {
				"fn": function( o ) {
					var nRealThs = o.nTHead.getElementsByTagName('th');
					var nScrollThs = nClonedThead.getElementsByTagName('th');
					for ( i=0, iLen=nRealThs.length ; i<iLen ; i++ )
					{
						nScrollThs[i].style.width = $(nRealThs[i]).width()+"px";
					}
				},
				"sName": "scrolling"
			} );
		 ...
		 * Notes:    The level indicator will be used in future versions of DataTables. At the time of
		 *   release for 1.7, only level 0 is used. 0 indicates and alert, while >0 would be for trace
		 *   and debug messages
		 */
		...
		return this.each(function()
		{
			...
			if ( typeof oInit != 'undefined' && oInit !== null )
			{
				...
				_fnMap( oSettings.oFeatures, oInit, "sScrollX" );
				_fnMap( oSettings.oFeatures, oInit, "sScrollY" );
			...
			{
				if ( $('tbody>tr:eq(0)', this).hasClass( oSettings.asStripClasses[i] ) ||
				     $('tbody>tr:eq(1)', this).hasClass( oSettings.asStripClasses[i] ) )
				{
					bStripeRemove = true;
					break;
				}
