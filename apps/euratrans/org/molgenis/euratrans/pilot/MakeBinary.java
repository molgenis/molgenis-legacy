			...
			for (Tuple line : new CsvFileReader(inputFile))
			{
				if (line_number != 0)
				{
					for (int columnIndex = 1; columnIndex < line.size(); columnIndex++)
					{
						...
						{
							elementLength.set(line.getString(columnIndex) != null ? line.getString(columnIndex)
									.length() : 0);
							System.out.println("First element, size: " + elementLength.get());
						}
						else
						{
							if (elementLength.get() != (line.getString(columnIndex) != null ? line.getString(
									columnIndex).length() : 0)) // nullpointer
																// ???
							{
								...
								System.out.println("Element " + line.getString(columnIndex) + " is not of length "
										+ elementLength.get());
								System.out
										.println("Element of unequal size found, exiting from function by throwing error");
								elementLength.set(0);
