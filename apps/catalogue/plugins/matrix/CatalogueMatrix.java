...
public class CatalogueMatrix extends EasyPluginController<CatalogueMatrixModel>
{
	...
	{
		return new FreemarkerView("CatalogueMatrixView.ftl", getModel()); //<plugin flavor="freemarker"
	}
		...
		try {
			...
			if(getModel().matrixViewerCat != null){
				...
				if(listMeas.size() > 0)
				{
					...
					getModel();
					getModel().matrixViewerCat = new MatrixViewer(this, CatalogueMatrixModel.getCATMATRIX(), 
							new SliceablePhenoMatrix<ObservationTarget, Measurement>(ObservationTarget.class, Measurement.class), 
