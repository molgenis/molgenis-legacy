...
/**
 * Class to load data from Ate's old AnimalDB into the new version with the
 * Pheno model.
 * 
 * @author erikroos
 * 
...
public class LoadAnimalDB
{
	...
	public void populateAnimal(String filename) throws Exception
	{
		...
		for (Tuple tuple : reader)
		{
			...
			if (!oldlocid.equals("NULL"))
			{
				...
				{
					// Check that the target is a Location, since there are also
					// Animals with OldAnimalDB ID's
	...
	public void populateAnimalsInExperiments(String filename) throws Exception
	{
		...
		for (Tuple tuple : reader)
		{
			...
			// animaltype -> should not be set here but when bringing in an
			// animal!
			/*
			 * String animaltype = tuple.getString("animaltype"); if
			 * (!animaltype.equals("NULL")) { if (animaltype.equals("1")) {
				...
				if (!dateString.equals("NULL"))
				{
					...
					if (eventType.equals("Died"))
					{
													...
													// time
						{
							...
							db.update(value);
							// Maybe TODO: get end status values from
							// somewhere and apply them?
						}
					}
					// Set the the cagecleaningevents based on the
