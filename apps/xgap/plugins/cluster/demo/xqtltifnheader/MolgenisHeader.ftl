<#--Date:        November 11, 2009
 * Template:	PluginScreenFTLTemplateGen.ftl.ftl
 * generator:   org.molgenis.generators.ui.PluginScreenFTLTemplateGen 3.3.2-testing
 * 
 * THIS FILE IS A TEMPLATE. PLEASE EDIT :-)
-->
<#macro plugins_cluster_demo_xqtltifnheader_MolgenisHeader screen>

<div style="height:10px;">&nbsp;</div>

<div id="sample_div" style="background-image:url(clusterdemo/bg/tifn_dark_logo_rp.png); width: 960px; height: 124px; line-height:7.5em;">
<font style="color: white; font-family: verdana,arial,sans-serif; font-weight: bold; letter-spacing: 0.15em; font-size: xx-large; font-variant: small-caps">
&nbsp;</font>
</div>


<div align="right">
	<font style="font-size:14px;">
		<#-->| <a href="api/REST/">JSON api</a> | <a href="api/SOAP/">SOAP api</a> | <a href="api/REST/">REST api</a> | -->
		<a target="_blank" href="http://www.molgenis.org/wiki/xQTL">Help</a> | <a href="api/R/">R api</a>
		  
	</font>
</div>



<div style="height:10px;">&nbsp;</div>

	
</#macro>
