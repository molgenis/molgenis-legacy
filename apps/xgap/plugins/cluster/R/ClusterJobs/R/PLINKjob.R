...
run_PLINK <- function(dbpath = "", subjob, item, jobid, outname, myanalysisfile, jobparams=list(c("inputname","hapmap1")), investigationname="", libraryloc=NULL){
  ...
  cat(Generate_Statement(paste("downloadFileViaCurl('",paste(dbpath,"/downloadfile",sep=""),"?name=",inputname,"_ped','run",jobid,"/",paste(inputname,item,sep=""),".ped')\n",sep="")),file=myanalysisfile,append=T)
  cat("report(2,\"PEDDownloaded\")\n",file=myanalysisfile,append=T)
  cat(Generate_Statement(paste("downloadFileViaCurl('",paste(dbpath,"/downloadfile",sep=""),"?name=",inputname,"_map','run",jobid,"/",paste(inputname,item,sep=""),".map')\n",sep="")),file=myanalysisfile,append=T)
  cat("report(2,\"MAPDownloaded\")\n",file=myanalysisfile,append=T)
  cat(Generate_Statement(paste("system(paste(\"plink --noweb --file run",jobid,"/",inputname,item," --assoc --pheno run",jobid,"/phenotypes.txt --mpheno ",item," --out \",outname,item,\"\",sep=\"\"))\n",sep="")),file=myanalysisfile,append=T)
