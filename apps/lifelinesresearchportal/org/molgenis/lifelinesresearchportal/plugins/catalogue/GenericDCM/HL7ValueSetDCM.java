...
public class HL7ValueSetDCM {
	...
	public HL7ValueSetDCM(Node node, XPath xpath) throws Exception {
		...
		valueSetsName = node.getAttributes().getNamedItem("name")
				.getNodeValue().trim();
		NodeList nodesFile = (NodeList) xpath.compile(CODE).evaluate(node,
				XPathConstants.NODESET);
		for (int i = 0; i < nodesFile.getLength(); i++) {
			HL7ValueSetAnswerDCM answer = new HL7ValueSetAnswerDCM(
					nodesFile.item(i), xpath);
