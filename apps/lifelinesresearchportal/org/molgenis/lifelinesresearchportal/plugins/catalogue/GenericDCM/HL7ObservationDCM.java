...
package org.molgenis.lifelinesresearchportal.plugins.catalogue.GenericDCM;
...
import org.molgenis.lifelinesresearchportal.plugins.hl7parser.HL7OntologyTerm;
...
public class HL7ObservationDCM {
	...
	public HL7ObservationDCM(Node measurement, XPath xpath) throws Exception {
		...
		NodeList ontologyTermCode = (NodeList) xpath.evaluate(
				OBSERVATION_ONTOLOGYCODE, measurement, XPathConstants.NODESET);

		hl7OntologyTermObservation = new ArrayList<HL7OntologyTerm>();
		for (int i = 1; i < ontologyTermCode.getLength(); i++) {
			HL7OntologyTerm ot = new HL7OntologyTerm(ontologyTermCode.item(i),
					xpath);
	...
	public void readDisplayName() throws Exception {
		...
		Node nameNode = (Node) xpath.evaluate(OBSERVATION_NAME, measurement,
				XPathConstants.NODE);
	...
	public void readID() throws Exception {
		...
		Node nameNode = (Node) xpath.evaluate(OBSERVATION_ID, measurement,
				XPathConstants.NODE);
	...
	public void readRepeatNumber() throws Exception {
		...
		Node nameNode1 = (Node) xpath.evaluate(OBSERVATION_REPEATNUMBER_LOW,
				measurement, XPathConstants.NODE);
		Node nameNode2 = (Node) xpath.evaluate(OBSERVATION_REPEATNUMBER_HIGH,
				measurement, XPathConstants.NODE);
		if (nameNode1 != null) {
	...
	public void readOriginalText() throws Exception {
		...
		Node nameNode = (Node) xpath.evaluate(OBSERVATION_DESCRIPTION,
				measurement, XPathConstants.NODE);
		if (nameNode != null) {
	...
	public void readValue() throws Exception {
		try {
			...
			Node nameNode = (Node) xpath.evaluate(OBSERVATION_DATATYPE,
					measurement, XPathConstants.NODE);
