...
public class HL7LLData implements HL7Data {
	...
	private NodeList readFile(String file, XPath xpath, String xpathExpres)
			throws Exception {
		...
		NodeList nodesFile = (NodeList) xpath.compile(xpathExpres).evaluate(
	...
	public HL7LLData(String file1, String file2) throws Exception {
		...
		for (int i = 0; i < nodesFile1.getLength(); i++) {
			...
			if (nodesFile1.item(i).getAttributes().getNamedItem("code")
					.getNodeValue().equals("Generic")) {
				hl7GenericDCM = new HL7GenericDCM(nodesFile1.item(i)
						.getParentNode(), xpath);

			} else if (nodesFile1.item(i).getAttributes().getNamedItem("code")
					.getNodeValue().equals("LRA")) {
				hl7StageLRA = new HL7StageLRA(nodesFile1.item(i)
						.getParentNode(), xpath);
			} else {
		...
		for (int i = 0; i < nodesFile2.getLength(); i++) {
			...
			if (listOfDCMObservation.contains(nodesFile2.item(i)
					.getAttributes().getNamedItem("name").getNodeValue())) {
				HL7ValueSetDCM valueSetDCM = new HL7ValueSetDCM(
						nodesFile2.item(i), xpath);
				hashValueSetDCM
						.put(valueSetDCM.getValueSetsName(), valueSetDCM);
				System.out.println("protocol " + i + ": "
						+ valueSetDCM.getValueSetsName());
				for (HL7ValueSetAnswerDCM r : valueSetDCM.getListOFAnswers()) {
					System.out.println("ValuesetAnswerDCM: " + r.getName());
				}
			} else {

				HL7ValueSetLRA valueSetLRA = new HL7ValueSetLRA(
						nodesFile2.item(i), xpath);
				hashValueSetLRA
						.put(valueSetLRA.getValueSetsName(), valueSetLRA);
				System.out.println("protocol " + i + ": "
						+ valueSetLRA.getValueSetsName());
