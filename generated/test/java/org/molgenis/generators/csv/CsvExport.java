...
public class CsvExport
{
	...
	{				
		...
		exportFeature(db, new File(directory+"/feature.txt"), skipAutoId ? Arrays.asList(new String[]{"Identifier","Name","__Type","description","dataType"}) : null, rules);		
		exportCategory(db, new File(directory+"/category.txt"), skipAutoId ? Arrays.asList(new String[]{"feature_Identifier"}) : null, rules);		
	...
	*/
	...
	{				
		for(List<? extends Entity> l: entityLists) if(l.size()>0)
		{
			...
			if(l.get(0).getClass().equals(Feature.class))
				exportFeature(l, new File(directory+"/feature.txt"));		
	...
	*/
	public void exportSpecial(File directory, Database db, boolean skipAutoId, List<Class<? extends Entity>> specialCases, boolean exclusion, QueryRule ... rules) throws Exception
	{
		if((exclusion && !specialCases.contains(Characteristic.class)) || (!exclusion && specialCases.contains(Characteristic.class)))
			{ exportCharacteristic(db, new File(directory+"/characteristic.txt"), skipAutoId ? Arrays.asList(new String[]{"Identifier","Name","__Type","description"}) : null, rules); }
		if((exclusion && !specialCases.contains(Feature.class)) || (!exclusion && specialCases.contains(Feature.class)))
			{ exportFeature(db, new File(directory+"/feature.txt"), skipAutoId ? Arrays.asList(new String[]{"Identifier","Name","__Type","description","dataType"}) : null, rules); }
	 ...
	 *  @param f the file to export to.
	 */
	public void exportFeature(Database db, File f, List<String> fieldsToExport, QueryRule ... rules) throws DatabaseException, IOException, ParseException, MolgenisModelException
	{
