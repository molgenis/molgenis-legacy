...
/**
 * DuplicatesPluginController takes care of all user requests and application logic.
 *
 * <li>Each user request is handled by its own method based action=methodName. 
 * <li> MOLGENIS takes care of db.commits and catches exceptions to show to the user
 * <li>DuplicatesPluginModel holds application state and business logic on top of domain model. Get it via this.getModel()/setModel(..)
 * <li>DuplicatesPluginView holds the template to show the layout. Get/set it via this.getView()/setView(..).
 */
public class DuplicatesPlugin extends EasyPluginController<DuplicatesPluginModel>
{
	public DuplicatesPlugin(String name, ScreenController<?> parent)
	{
		super(name, null, parent);
		this.setModel(new DuplicatesPluginModel(this)); //the default model
		this.setView(new FreemarkerView("DuplicatesPluginView.ftl", getModel())); //<plugin flavor="freemarker"
	}
