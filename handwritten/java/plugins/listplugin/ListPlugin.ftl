...
<div id='top_part'>
	...
	<select name="targettype" id="targettype" onchange="fnRemAllFeatures()">
...
<!-- Handler for removing all features -->
function fnRemAllFeatures() {
	var oSettings = oTable.fnSettings();
	oTable.fnClearTable(0);
	
	// Temporarily attach new 'fnServerData' for removing columns
	oSettings.fnServerData = function (sSource, aoData, fnCallback) {
		aoData.push( { "name": "feature", "value": -1 } );
		aoData.push( { "name": "printValInfo", "value": document.getElementById('datetimetoggle').checked } );
		aoData.push( { "name": "limitVal", "value": document.getElementById('limitvaltoggle').checked } );
		aoData.push( { "name": "targetType", "value": document.getElementById('targettype').value } );
		aoData.splice(3, 1, { "name": "iDisplayStart", "value": storedDisplayStart });
		...
		jQuery.getJSON( sSource, aoData, function (json) {
			...
			oSettings.aoColumns[0].sTitle = json.asFeatureNames[0];
			nTh = oSettings.aoColumns[0].nTh;
			nTh.innerHTML = oSettings.aoColumns[0].sTitle;
			nTf = oSettings.aoColumns[0].nTf;
			var filterText;
			var className = "";
			if (json.asFilterTerms[0] != "") {
				filterText = json.asFilterTerms[0];
				oSettings.aoPreSearchCols[0].sSearch = filterText;
			} else {
				filterText = "Filter " + json.asFeatureNames[0];
				className = "search_init";
			}
			nTf.innerHTML = "<input type='text' name='search_feat0' value= '" + filterText + "' class='" + className + "' onkeyup='oTable.fnFilter(this.value, 0' onclick='if ( this.className == \"search_init\" ){ this.className = \"\"; this.value = \"\";}' />";
...
<!-- Handler for updating the table without adding or removing a feature -->
